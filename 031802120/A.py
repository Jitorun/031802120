    ...
    for i in range(0, num):
        for j in range(0, num):
            ...
            for d in direction:  # 四个偏移方向
               ...
               # if gn== 9 and flag==1:
               #     state[lines[1]//3][lines[1]%3], state[lines[2]//3][lines[2]%3] = state[lines[2]//3][lines[2]%3], state[lines[1]//3][lines[1]%3]
               #     print("这里是用来测试length的地方",gn)
               #     flag=0
               #     print(lines[0])
...
if __name__=='__main__':
    ...
    print(GOAL)
    lines = []
    with open("json_get.txt", 'r') as file_to_read:
        while True:
            js_line = file_to_read.readline()
            if not js_line:
                break
            js_line = js_line.strip('\n')
            lines.append(js_line)
    print(lines)
    #goldnum(win[], x2, x3, x4, x5, x6, x7, x8, x9)
    #goldnum(win[0], win[1], win[2], win[3], win[4], win[5], win[6], win[7], win[8])
